'use client'

import { motion } from 'framer-motion'
import Counter from './ui/Counter'

const container = {
  hidden: {},
  show: { transition: { staggerChildren: 0.15 } },
}
const item = {
  hidden: { opacity: 0, y: 22 },
  show:   { opacity: 1, y: 0, transition: { duration: 0.58, ease: 'easeOut' } },
}

export default function HeroAnimations() {
  return (
    <div className="relative z-10 flex flex-col min-h-screen">
      <div className="h-20" />

      {/* Main content — bottom-left */}
      <div className="flex-1 flex items-end pb-10 sm:pb-16 lg:pb-20 px-6 sm:px-12 md:px-20 lg:px-28">
        <motion.div
          className="max-w-sm"
          variants={container}
          initial="hidden"
          animate="show"
        >
          {/* Badge — naranja se mantiene, contrasta bien sobre video */}
          <motion.div
            variants={item}
            className="inline-flex items-center gap-1.5 text-[14px] font-medium mb-3 group"
            style={{ color: '#FF5500' }}
          >
            <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: '#FF5500' }} />
            Fabricación bajo demanda · Envío 48h
            <span className="inline-block transition-transform duration-200 group-hover:translate-x-0.5">→</span>
          </motion.div>

          {/* H1 */}
          <motion.h1
            variants={item}
            className="text-[1.75rem] sm:text-[2.25rem] leading-[1.15] font-medium tracking-tight mb-3"
            style={{ color: '#ffffff', textShadow: '0 2px 16px rgba(0,0,0,0.5)' }}
          >
            Del diseño digital a la pieza real,{' '}
            <span style={{ color: '#FF5500' }}>sin límites.</span>
          </motion.h1>

          {/* Subtítulo */}
          <motion.p
            variants={item}
            className="text-[16px] font-normal mb-4 leading-snug max-w-[260px]"
            style={{ color: 'rgba(255,255,255,0.82)', textShadow: '0 1px 8px rgba(0,0,0,0.4)' }}
          >
            Impresión 3D para empresas, ingenieros y makers. Calidad industrial.
          </motion.p>

          {/* CTA */}
          <motion.div variants={item}>
            <a
              href="#contacto"
              className="inline-flex items-center gap-2 text-[16px] font-medium rounded-full px-5 py-2.5 transition-all duration-200 group"
              style={{ color: '#ffffff', border: '1px solid rgba(255,255,255,0.5)', textShadow: '0 1px 4px rgba(0,0,0,0.3)' }}
              onMouseEnter={e => {
                ;(e.currentTarget as HTMLElement).style.background = '#FF5500'
                ;(e.currentTarget as HTMLElement).style.borderColor = '#FF5500'
                ;(e.currentTarget as HTMLElement).style.color = '#fff'
              }}
              onMouseLeave={e => {
                ;(e.currentTarget as HTMLElement).style.background = 'transparent'
                ;(e.currentTarget as HTMLElement).style.borderColor = 'rgba(255,255,255,0.5)'
                ;(e.currentTarget as HTMLElement).style.color = '#ffffff'
              }}
            >
              Empieza tu proyecto
              <span className="inline-block transition-transform duration-200 group-hover:translate-x-0.5">→</span>
            </a>
          </motion.div>
        </motion.div>
      </div>

      {/* Stat — esquina superior derecha */}
      <div className="absolute right-6 sm:right-24 top-[22%] text-right">
        <div className="flex items-center gap-3 justify-end">
          <div className="hidden md:block h-px w-20 rotate-[20deg]"
               style={{ background: 'rgba(255,85,0,0.45)' }} />
          <Counter
            value={4200}
            suffix="+"
            className="text-4xl sm:text-5xl font-medium tracking-tight"
            style={{ color: '#FF5500', textShadow: '0 2px 12px rgba(255,85,0,0.25)' }}
          />
        </div>
        <p className="text-xs sm:text-sm mt-1"
           style={{ color: 'rgba(255,255,255,0.65)', textShadow: '0 1px 6px rgba(0,0,0,0.35)' }}>
          proyectos completados
        </p>
      </div>

      {/* Stats — esquina inferior derecha — 50+ y 98% apilados */}
      <div className="absolute right-6 sm:right-20 bottom-16 sm:bottom-20 text-right flex flex-col gap-6 items-end">

        {/* 50+ materiales */}
        <div>
          <div className="flex items-center gap-3 justify-end">
            <div className="hidden md:block h-px w-20 rotate-[-20deg]"
                 style={{ background: 'rgba(255,85,0,0.45)' }} />
            <Counter
              value={50}
              suffix="+"
              className="text-4xl sm:text-5xl font-medium tracking-tight"
              style={{ color: '#FF5500', textShadow: '0 2px 12px rgba(255,85,0,0.25)' }}
            />
          </div>
          <p className="text-xs sm:text-sm mt-1"
             style={{ color: 'rgba(255,255,255,0.65)', textShadow: '0 1px 6px rgba(0,0,0,0.35)' }}>
            materiales disponibles
          </p>
        </div>

        {/* 98% satisfacción */}
        <div>
          <div className="flex items-center gap-3 justify-end">
            <div className="hidden md:block h-px w-20 rotate-[-20deg]"
                 style={{ background: 'rgba(255,85,0,0.45)' }} />
            <Counter
              value={98}
              suffix="%"
              className="text-4xl sm:text-5xl font-medium tracking-tight"
              style={{ color: '#FF5500', textShadow: '0 2px 12px rgba(255,85,0,0.25)' }}
            />
          </div>
          <p className="text-xs sm:text-sm mt-1"
             style={{ color: 'rgba(255,255,255,0.65)', textShadow: '0 1px 6px rgba(0,0,0,0.35)' }}>
            satisfacción cliente
          </p>
        </div>

      </div>
    </div>
  )
}
